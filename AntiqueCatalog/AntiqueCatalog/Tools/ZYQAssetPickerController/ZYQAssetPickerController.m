...
-(void)drawRect:(CGRect)rect{
    ...
    CGSize titleSize        = [self.text sizeWithAttributes:fontDic];
    [self.textColor set];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldTap)]) {
        if (![_delegate shouldTap]&&!_selected) {
			UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"最多选9张图片" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
			[alert show];
            return;
        }
    }
...
- (void)viewDidLoad
{
	...
	self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT-(UI_NAVIGATION_BAR_HEIGHT)) style:UITableViewStylePlain];
	...
	UIButton *quickBtn = [[UIButton alloc]initWithFrame:CGRectMake(12, 18, 44, 20)];
	quickBtn.backgroundColor = [UIColor clearColor];
	[quickBtn setTitle:@"预览" forState:0];
	[quickBtn addTarget:self action:@selector(quickLook) forControlEvents:UIControlEventTouchUpInside];
	[quickBtn setTitleColor:BLUECOLOR forState:0];
	[view addSubview:quickBtn];
	
	UIButton *finishBtn = [[UIButton alloc]initWithFrame:CGRectMake(UI_SCREEN_WIDTH-56, 18, 44, 20)];
	...
	[finishBtn setTitleColor:BLUECOLOR forState:0];
	[view addSubview:finishBtn];
	
	redView = [[UIImageView alloc]initWithFrame:CGRectMake(250, 8, 19, 19)];
	redView.layer.masksToBounds = YES;
	redView.layer.cornerRadius = 9.5;
	redView.hidden = YES;
	redView.backgroundColor = [UIColor redColor];
	[view addSubview:redView];
	
	countLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 2, 19, 15)];
...
-(void)quickLook{
	...
	scanVC.imgArr = _indexPathsForSelectedItems;
	scanVC.index = 0;
	scanVC.isPhoto = YES;
...
- (void)setupAssets
{
	...
	self.navigationController.navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObject:BLUECOLOR forKey:NSForegroundColorAttributeName];
...
//    self.title = [NSString stringWithFormat:format, (long)indexPaths.count];
	...
	if (indexPaths.count > 0) {
		redView.hidden = NO;
		countLabel.text = [NSString stringWithFormat:format, (long)indexPaths.count];
	}else{
		redView.hidden = YES;
	}
