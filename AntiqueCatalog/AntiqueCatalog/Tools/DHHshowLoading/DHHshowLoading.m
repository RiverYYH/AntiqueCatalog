...
+ (void)showLoadingForView:(UIView *)view allowUserInteraction:(BOOL)allowUserInteraction
{
    ...
    DHHshowLoading *loadingView = [[self alloc] initWithView:view isHudView:NO];
...
+ (void)showGrayLoadingForView:(UIView *)view allowUserInteraction:(BOOL)allowUserInteraction
{
    ...
    DHHshowLoading *loadingView = [[self alloc] initWithView:view isHudView:YES];
    loadingView.userInteractionEnabled = !allowUserInteraction;
    loadingView.alpha = 0;
    [view addSubview:loadingView];
    [loadingView showViewWithAnimate:YES];
...
{
    DHHshowLoading *gearLoading = [self gearLoadingForView:view];
    if (gearLoading != nil) {
...
+ (instancetype)gearLoadingForView:(UIView *)view
{
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            [array addObject:(DHHshowLoading *)subview];
        }
    }
    if (array.count>1) {
        for (NSUInteger j = 0; j < array.count; j++) {
            DHHshowLoading *loadingView = (DHHshowLoading *)array[j];
            if (j != array.count - 1) {
                [loadingView removeFromSuperview];
            }
            if (j == array.count - 1){
                return (DHHshowLoading *)array[j];
            }
        }
    }else {
        return (DHHshowLoading *)array.firstObject;
    }
    return nil;
}
