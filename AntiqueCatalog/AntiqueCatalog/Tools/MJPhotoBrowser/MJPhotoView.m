...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(alertViewTap:)];
...
//			[_imageView sd_setImageWithPreviousCachedImageWithURL:_photo.url andPlaceholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
//				
...
- (void)alertViewTap:(UITapGestureRecognizer *)recognizer
{
    if (sdoptionsview == NULL) {
        sdoptionsview = [[SDOptionsView alloc]init];
        sdoptionsview.frame = CGRectMake(CGRectGetMinX(_imageView.frame), 0, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT);
        [sdoptionsview block:^(NSInteger clickIndex) {
            [self alertViewTapint:clickIndex];
        }];
        [self addSubview:sdoptionsview];
    }
//    NSLog(@"1111111");
    
}

- (void)alertViewTapint:(NSInteger)clickIndex
{
    switch (clickIndex) {
        case 0:
        {
            [sdoptionsview removeFromSuperview];
            sdoptionsview = nil;
            if ([self.photoViewDelegate respondsToSelector:@selector(photoViewKnowledgeChart:)]) {
                [self.photoViewDelegate photoViewKnowledgeChart:[_photo.url absoluteString]];
            }
        }
            break;
        case 1:
