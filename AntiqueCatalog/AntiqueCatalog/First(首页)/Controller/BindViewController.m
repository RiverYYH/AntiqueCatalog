        ...
        case 101:
        {
            if (STRING_NOT_EMPTY(_phoneTextField.text))
            {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [Api requestWithMethod:@"GET" withPath:API_URL_User_Send_Code withParams:params withSuccess:^(id responseObject){
        ...
        case 102:
        {
            if (STRING_NOT_EMPTY(_phoneTextField.text)&&STRING_NOT_EMPTY(_verificationTextField.text))
            {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    [Api requestWithMethod:@"GET" withPath:API_URL_User_Phone withParams:params withSuccess:^(id responseObject){
                        if ([[responseObject objectForKey:@"status"] intValue] == 1)
                        {
                            [self hideHud];
