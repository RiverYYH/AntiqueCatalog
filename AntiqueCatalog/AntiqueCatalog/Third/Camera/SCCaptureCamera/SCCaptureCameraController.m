...
#define CAMERA_MENU_VIEW_HEIGH  80  //menu

//color
#define bottomContainerView_UP_COLOR     [UIColor blackColor]       //bottomContainerView的上半部分
#define bottomContainerView_DOWN_COLOR   [UIColor blackColor]       //bottomContainerView的下半部分
...
- (void)viewDidLoad
{
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        self.previewRect = CGRectMake(0, 80, UI_SCREEN_WIDTH, UI_SCREEN_WIDTH);
    }
...
- (void)addbottomContainerView {
    ...
    CGRect bottomFrame = CGRectMake(0, CGRectGetMaxY(_captureManager.previewLayer.frame), SC_APP_SIZE.width, UI_SCREEN_HEIGHT-CGRectGetMaxY(_captureManager.previewLayer.frame));
...
//    CGFloat downH = (isHigherThaniPhone4_SC ? CAMERA_MENU_VIEW_HEIGH : 0);
    ...
    [self buildButton:CGRectMake((UI_SCREEN_WIDTH - cameraBtnLength) / 2, (_bottomContainerView.frame.size.height - CAMERA_MENU_VIEW_HEIGH - cameraBtnLength) / 2 +20, cameraBtnLength, cameraBtnLength)
         normalImgStr:@"paishe"
      highlightImgStr:@"paishe"
       selectedImgStr:@""
               action:@selector(takePictureBtnPressed:)
           parentView:_bottomContainerView];
	
	UIButton *photoBtn = [UIButton buttonWithType:UIButtonTypeCustom];
	photoBtn.frame = CGRectMake((SC_APP_SIZE.width - cameraBtnLength) / 2+120, (_bottomContainerView.frame.size.height - CAMERA_MENU_VIEW_HEIGH - cameraBtnLength) / 2 +40, 50, 50);
    [photoBtn setImage:[UIImage imageNamed:@"xuantu"] forState:0];
	[photoBtn addTarget:self action:@selector(btnClick) forControlEvents:UIControlEventTouchUpInside];
	[_bottomContainerView addSubview:photoBtn];
...
//    CGFloat menuViewY = (isHigherThaniPhone4_SC ? SC_DEVICE_SIZE.height - CAMERA_MENU_VIEW_HEIGH : 0);
    UIView *menuView = [[UIView alloc] initWithFrame:CGRectMake(0, IOS7?0:0, self.view.frame.size.width, CAMERA_MENU_VIEW_HEIGH)];
...
-(void)btnClick{
	...
	UzysAssetsPickerController *picker1 = [[UzysAssetsPickerController alloc] init];
	picker1.delegate = self;
	picker1.maximumNumberOfSelectionPhoto = 9;
...
- (void)UzysAssetsPickerController:(UzysAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets{
	...
	[self dismissViewControllerAnimated:YES completion:^{
		...
		if (iOSVersion<8) {
			[self dismissViewControllerAnimated:YES completion:^{
				[[NSNotificationCenter defaultCenter]postNotificationName:@"post_photo_weibo" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:array,@"photo", nil]];
			}];
		}else{
...
-(void)assetPickerController:(ZYQAssetPickerController *)picker didFinishPickingAssets:(NSArray *)assets{
	[self dismissViewControllerAnimated:YES completion:^{
		...
		if (_isPost2) {
			if (iOSVersion<8) {
				[self dismissViewControllerAnimated:YES completion:^{
					[[NSNotificationCenter defaultCenter]postNotificationName:@"post_photo_weibo1" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:array,@"photo", nil]];
				}];
			}else{
				[self.presentingViewController dismissViewControllerAnimated:YES completion:^{
					[[NSNotificationCenter defaultCenter]postNotificationName:@"post_photo_weibo1" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:array,@"photo", nil]];
...
- (void)addMenuViewButtons {
    ...
    [SCCommon drawALineWithFrame:CGRectMake(eachW, 20, 1, 40) andColor:rgba_SC(102, 102, 102, 1.0000) inLayer:_cameraMenuView.layer];
...
//拍完照后的遮罩
- (void)addCameraCover {
    UIView *upView = [[UIView alloc] initWithFrame:CGRectMake(0, 80, UI_SCREEN_WIDTH, 0)];
    upView.backgroundColor = [UIColor blackColor];
    [self.view addSubview:upView];
    self.doneCameraUpView = upView;
    
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_bottomContainerView.frame), UI_SCREEN_WIDTH, 0)];
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        upFrame.size.height = (toShow ? UI_SCREEN_WIDTH / 2 : 0);
        _doneCameraUpView.frame = upFrame;
        
        CGRect downFrame = _doneCameraDownView.frame;
        downFrame.origin.y = (toShow ? UI_SCREEN_WIDTH / 2 + 80 : CGRectGetMaxY(_bottomContainerView.frame));
        downFrame.size.height = (toShow ? UI_SCREEN_WIDTH / 2 : 0);
